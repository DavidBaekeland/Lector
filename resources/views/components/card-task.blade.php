...
<div class="card card-large card-task" x-data="{ open: false }">
    <div class="announcement-title-active">
        ...
        @elseif(auth()->user()->hasTaskSubmitted($task->id))
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="task-icon">
                <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
            </svg>
        @elseif($task->deadline < now() && !auth()->user()->hasTaskSubmitted($task->id))
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="task-icon">
    ...
    @if($task->deadline >= now() && auth()->user()->can("upload_task", $task))
        @if(!auth()->user()->hasTaskSubmitted($task->id))
            <x-primary-button class="task-button" x-on:click="open = ! open">Indienen</x-primary-button>
        @else
            <x-primary-button class="task-button" x-on:click="open = ! open">Opnieuw indienen</x-primary-button>
        @endif
